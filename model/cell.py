...
class DCGRUCell(torch.nn.Module):
    def __init__(self, num_units, max_diffusion_step, num_nodes, nonlinearity='tanh',
        ...
        self.ran_initgraph = torch.eye(self._num_nodes).to(device)
    ...
    def _calculate_random_walk_matrix(self, adj_mx):
        ...
        adj_mx = adj_mx + self.ran_initgraph
    ...
    def forward(self, inputs, hx, adj):
        ...
        adj_new = self._calculate_random_walk_matrix(adj.t()).t()

        supports = [adj_mx, adj_new]
        ...
        if self._use_gc_for_ru:
            ...
            fn = self._fc
        value = torch.sigmoid(fn(inputs, supports, hx, output_size, bias_start=1.0))
        value = torch.reshape(value, (-1, self._num_nodes, output_size))
        r, u = torch.split(tensor=value, split_size_or_sections=self._num_units, dim=-1)
        r = torch.reshape(r, (-1, self._num_nodes * self._num_units))
        u = torch.reshape(u, (-1, self._num_nodes * self._num_units))

        c = self._gconv(inputs, supports, r * hx, self._num_units)
    ...
    def _gconv(self, inputs, supports, state, output_size, bias_start=0.0):
        ...
        if self._max_diffusion_step == 0:
            ...
            for support in supports:
                ...
                for k in range(2, self._max_diffusion_step + 1):
                    ...
                    x = self._concat(x, x2)
                    x1, x0 = x2, x1
        num_matrices = 2 * self._max_diffusion_step + 1  # Adds for x itself.
